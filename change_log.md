[12/12/2025 16:25] - src/components/analysis/AnalysisVidaView.tsx / AnalysisNecessidadesView.tsx - Tooltips Explicativos: Adicionados tooltips detalhados em TODOS os KPIs e análises para explicar: (1) O que cada indicador significa; (2) Como é calculado; (3) Qual ação tomar. Inclui cursor-help para indicar hover. Campos: Críticas, Em Atenção, Saudáveis, EOL 30d, Seq. Ativas, Utilização, Score de Risco, Sala de Guerra, Previsão de Gargalos, Single Point of Failure, Distribuição de Capacidade, Prioridade Alta/Média/Baixa, Reposição 30d, Demanda Crescente, Ações Recomendadas - Cascade
[12/12/2025 16:10] - src/components/analysis/AnalysisVidaView.tsx - Análises Estratégicas Avançadas: (1) Score de Risco Composto (0-100) combinando cobertura, desgaste, demanda e single-seq; (2) Sala de Guerra com Top 10 matrizes críticas ranqueadas; (3) Previsão de Gargalos (30/60/90 dias); (4) Single Point of Failure (matrizes vulneráveis com 1 seq); (5) Distribuição de Capacidade por faixa de risco; (6) Plano de Ação Recomendado com ações urgentes, redundância e inspeção - Cascade
[12/12/2025 15:51] - src/components/analysis/AnalysisVidaView.tsx / AnalysisNecessidadesView.tsx - Modernização das abas Vida e Necessidades com painéis gerenciais inteligentes: (1) KPIs estratégicos (críticas, atenção, saudáveis, EOL 30d, seq. ativas, utilização); (2) Insights automáticos com alertas de ruptura, cobertura baixa, demanda crescente e sequências insuficientes; (3) Ações recomendadas (confeccionar, ampliar capacidade, revisar planejamento); (4) UI moderna com cards coloridos e indicadores visuais - Cascade
[28/11/2025 12:37] - src/components/analysis/AnalysisProdutividadeView.tsx / src/utils/productivityAnalysis.ts / docs/* - Implementada análise completa de produtividade com gráficos interativos (linha, barras, pizza); criada documentação completa para planejamento mobile-first (8 documentos); melhorias de responsividade no LoginDialog e Index.tsx - Cascade
...
[14/11/2025 11:40] - src/components/analysis (Carteira/Vida) / database_schema.md / change_log.md - Lookup de "Último Pedido" migrado para a VIEW `analysis_carteira_last_implant` com chaves normalizadas; correção de normalização na aba Vida; documentação da VIEW e índices auxiliares registrada - Cascade
[14/11/2025 11:58] - data_schema.sql - Correção crítica: adicionada criação da tabela `analysis_carteira_flat` e RPC `analysis_carteira_flat_truncate()` que estavam faltando antes da VIEW e índices; ajuste da migração completa no bloco BEGIN/COMMIT - Cascade
[14/11/2025 12:30] - src/components/analysis/FerramentaAnalysisDialog.tsx - Ajuste na Análise de Extremes: passa a considerar apenas produções com volume ≥ 200 kg para calcular menor produtividade e volume associado; descrição atualizada no rodapé do diálogo - Cascade
[14/11/2025 13:35] - src/components/AnalysisView.tsx / src/components/analysis - Modal de Análise de Ferramenta migrado para aba dedicada em `AnalysisView`, com navegação via botão "Analisar Ferramenta" na aba Produção e botão "Voltar para Produção" na aba de análise; adicionados gráficos de produtividade (12 meses, linhas de objetivo 1.300/1.000 kg/h) e de entradas de pedido da carteira, com tooltip exibindo sequência apenas na linha de produtividade - Cascade
[14/11/2025 13:37] - src/components/analysis/KeywordsManagerDialog.tsx - Botão "Adicionar Todas" do Gerenciador de Palavras-Chave passou a remover duplicatas dentro do próprio lote e ignorar palavras já existentes em `analysis_keywords`, preservando os registros atuais - Cascade
